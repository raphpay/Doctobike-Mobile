class Dates {
  ...
  static formatDate(
    ...
    console.log("1", date);
    // Validate the input and parse if necessary
    if (!date) {
      throw new Error("Invalid date: Date is undefined or null.");
    }
    ...
    if (isNaN(parsedDate.getTime())) {
      throw new Error("Invalid date: Could not parse the date.");
    }
    ...
    // Access month names based on the language
    const months = this.monthNames?.[language];
    if (!months || !Array.isArray(months) || months.length !== 12) {
      throw new Error(
        `Invalid month names: Ensure that "monthNames" is defined correctly for language "${language}".`
      );
    }
